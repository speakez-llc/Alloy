    ...
    testList "Result" [
        testList "Basic Operations" [
            testCase "map transforms Ok value correctly" <| fun _ ->
                let input: Result<int, string> = Ok 5
                let expected: Result<int, string> = Ok 10
            ...
            testCase "mapError transforms Error value correctly" <| fun _ ->
                let input: Result<int, string> = Error "error"
                let expected: Result<int, string> = Error "ERROR"
                let result: Result<int, string> = Result.mapError (fun (e: string) -> e.ToUpper()) input
            ...
            testCase "bind can produce Error from Ok" <| fun _ ->
                let input: Result<int, string> = Ok 5
                let expected: Result<int, string> = Error "computed error"
        ...
        testList "Value Extraction" [
            ...
            testCase "defaultWith applies function to Error" <| fun _ ->
                let input: Result<int, string> = Error "error"
                let result: int = Result.defaultWith (fun (e: string) -> e.Length) input
        ...
        testList "Check Functions" [
            testCase "isOk/isError detect result type correctly" <| fun _ ->
                let okResult: Result<int, string> = Ok 5
                let errorResult: Result<int, string> = Error "error"
        ...
        testList "Advanced Operations" [
            testCase "bimap transforms both cases correctly" <| fun _ ->
                ...
                let okResult = Result.bimap (fun x -> x * 2) (fun (e: string) -> e.ToUpper()) okInput
                Expect.equal okResult (Ok 10) "bimap should transform Ok value"
                
                let errorResult = Result.bimap (fun x -> x * 2) (fun (e: string) -> e.ToUpper()) errorInput
            ...
            testCase "filter checks Ok value against predicate" <| fun _ ->
                ...
                let errorInput: Result<int, string> = Error "original error"
            ...
            testCase "Complex error handling flow" <| fun _ ->
                ...
                let safeDivide (x: float) (y: float) : Result<float, string> =
                ...
                let errorHandled =
                    failDivide 
                    |> Result.mapError (fun (e: string) -> $"Calculation error: {e}")
